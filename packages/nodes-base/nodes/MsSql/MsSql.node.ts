...
export class MsSql implements INodeType {
	description: INodeTypeDescription = {
		...
		group: ['input'],
		version: 1,
		description: 'Gets, add and update data in MsSQL.',
		defaults: {
			name: 'MsSQL',
			color: '#4279a2',
		},
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		const credentials = this.getCredentials('msSql');
		...
		const connection = new mssql.ConnectionPool(credentials); 
		...
		if (operation === 'executeQuery') {
			...
			let queryResult = await request.query(rawQuery);

			returnItems = this.helpers.returnJsonArray(queryResult.recordset as IDataObject[]);
